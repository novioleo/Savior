...
def db_post_process(_predict_score, _thresh, _bbox_scale_ratio, _min_size=3):
    ...
    if cv2.__version__.startswith('3'):
        _, contours, _ = cv2.findContours(mask_region, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
    elif cv2.__version__.startswith('4'):
        contours, _ = cv2.findContours(mask_region, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
    else:
        raise NotImplementedError(f'opencv {cv2.__version__} not support')
    for m_contour in contours:
        if len(m_contour) < 4:
            continue
        m_rotated_box = get_min_area_bbox(mask_region, m_contour)
        m_box_width = m_rotated_box['box_width']
        m_box_height = m_rotated_box['box_height']
        if min(m_box_width * w, m_box_height * h) < _min_size:
            continue
        if _bbox_scale_ratio != 1:
            m_scaled_bbox = m_rotated_box.copy()
            m_scaled_bbox['box_width'] *= _bbox_scale_ratio
            m_scaled_bbox['box_height'] *= _bbox_scale_ratio
            to_return_boxes.append(m_scaled_bbox)
        else:
