...
async def general_ocr(
    ...
    download_image_task = DownloadImageFromURLServiceTask()
    download_image_task.add_dependency_from_value('_image_url', image_url)
    text_detect_task = TextDetectServiceTask()
    ...
    for i in range(detect_result['box_count']):
        m_recognize_task = TextRecognizeServiceTask(_task_name=f'No. {i} recognize')
    ...
    recognize_result = await ServiceTask.wait_and_compose_all_task_result(*recognize_task)
    # 上传结果图片，这个环节可以直接在interface中实现，但是为了保证服务的并发量，所以依然是将任务分发出去
    visualization_task = OCRResultVisualizationServiceTask()
